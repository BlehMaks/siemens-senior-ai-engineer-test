...
| Cloud Run GPU | Variable single-GPU demand passes quota, cold-start, model-load, license, residency, and cost gates | Utilization is sustained, multi-GPU, shared, or topology-aware | Quality/latency benchmark, quota reservation, fallback, security review, teardown |
...
| Regional production cell | 99.99% monthly submission/status if business impact justifies it | Per-query-class completion and quality SLOs; queue age and model/destination saturation are leading signals | Candidate RTO 60 min / RPO 5 min, proven for run, event, idempotency, quota, and governed memory state | Business continuity + SRE + platform data owner |
