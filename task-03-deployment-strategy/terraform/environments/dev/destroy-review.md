...
- The Cloud Tasks dispatch queue has `prevent_destroy` so queued work is not
  removed by a failed routine teardown.
...
Cloud Run deletion protection and queue `prevent_destroy` must also be disabled
before removing the execution plane. These explicit changes keep destructive
intent out of routine environment cleanup.
