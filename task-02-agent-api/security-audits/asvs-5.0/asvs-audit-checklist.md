...
- Focused relevant requirements assessed below: 33
- Pass: 26
- Fail: 0
- Partial: 6
- N/A: 1
- Unknown: 0 for local Task 2 scope
- Critical findings: 0
- High findings: 0
- Residual risk: production-only ingress, TLS, IAM, secret storage, image/SBOM,
  log retention, backup, and DDoS controls remain Task 3/release evidence.
...
The focused matrix below is the filled Task 2 audit evidence. The remaining
generated checklist is preserved as the full ASVS 5.0 requirement corpus so later
Task 3/release audits can expand the scope without losing requirement text.

## Focused Task 2 ASVS Decision Matrix

| ASVS ID | Status | Rationale and evidence |
| :--- | :--- | :--- |
| V1.2.4 | Pass | SQLite access uses parameter binding rather than string-interpolated user data; injection-shaped labels and queries remain data in `tests/security/test_api_attack_surface.py`. |
| V1.2.5 | N/A | Task 2 exposes no shell, plugin, template, or arbitrary code execution path. |
| V1.3.6 | Pass | The imported Task 1 executor boundary owns SSRF checks; Task 2 stores and returns only validated public citation URLs. |
| V2.1.1 | Pass | Public request bodies use strict Pydantic models and extra fields fail closed before side effects. |
| V2.1.3 | Pass | Labels, queries, cursors, IDs, idempotency keys, and correlation IDs are type/length bounded at route/schema boundaries. |
| V2.2.1 | Pass | Session and run creation derive tenant identity from authentication, not caller-supplied fields; mass-assignment tests cover forged identity/state fields. |
| V2.3.2 | Pass | Run state transitions use repository transactions, versions, and leases; stale or wrong-lease writers cannot overwrite terminal state. |
| V2.3.3 | Pass | Quota admission and idempotent run creation happen transactionally before work dispatch. |
| V2.3.4 | Pass | Cancellation and worker cleanup preserve terminal state and tolerate cleanup faults without re-opening work. |
| V2.4.1 | Pass | Automated workers validate stored run identity, lease ownership, and executor output before persistence. |
| V8.1.1 | Pass | Route scopes distinguish session read/write, run read/write, and memory delete functions. |
| V8.1.2 | Pass | Administrative key operations require local CLI/operator context and do not expose public HTTP admin routes. |
| V8.2.1 | Pass | Object reads, streams, cancels, deletes, and repository operations repeat authenticated tenant predicates. |
| V8.2.2 | Pass | Cross-tenant route and repository tests reject sibling object access. |
| V8.2.3 | Pass | Worker lease claims and terminal writes bind run identity to tenant/session/run IDs. |
| V8.3.1 | Pass | Public response models omit tenant internals, key hashes, leases, storage paths, prompts, and reasoning. |
| V8.3.2 | Pass | Unknown public input fields are forbidden; tests reject tenant_id, run_id, state, answer, prompts, and budget overrides. |
| V8.4.1 | Pass | Tenant deletion and memory deletion paths are scoped by authenticated or operator-provided tenant identity. |
| V11.2.1 | Pass | API keys are generated from high-entropy random material and only the plaintext creation result is shown once. |
| V11.4.3 | Pass | API-key verification stores HMAC-SHA256 digests with a 32-byte pepper and constant-time comparison. |
| V11.5.1 | Partial | Pepper rotation and managed secret storage are production controls; local tests verify required pepper strength. |
| V12.1.1 | Partial | TLS termination, HSTS, and ingress security headers are Task 3 deployment controls, not local ASGI behavior. |
| V13.1.3 | Partial | Local config fails closed for missing/weak pepper; Secret Manager sourcing belongs to Task 3. |
| V13.2.4 | Partial | Local API has no direct outbound HTTP, but the deployment must wire only the reviewed Task 1 executor or equivalent SSRF controls. |
| V13.3.1 | Partial | DDoS absorption and global unauthenticated throttling require Cloud Armor/ingress evidence in Task 3. |
| V14.1.2 | Pass | Public errors, status, SSE, and telemetry avoid raw keys, tenant IDs, prompts, reasoning, and private query content. |
| V14.2.1 | Partial | SQLite encryption and backup handling are operator/deployment controls outside the local submission profile. |
| V15.1.3 | Pass | Bounded request bodies, queue limits, execution budgets, leases, and cancellation guard resource use. |
| V15.2.2 | Pass | Task 1 redirect/content/decompression limits are the only approved outbound-fetch boundary for the executor. |
| V15.3.2 | Pass | Readiness validates the same read-only database file identity exposed at the configured path, including symlink and inode-swap cases. |
| V15.4.2 | Pass | Race-sensitive state changes use optimistic versions, leases, and tenant-scoped idempotency tests. |
| V16.1.1 | Pass | Security-relevant key lifecycle, quota, run, cancellation, and error events are recorded with pseudonymous correlation. |
| V16.5.4 | Pass | Error envelopes are generic, bounded, and correlation-bearing without leaking internal exception details. |
...
<!-- ASVS-REQ:V1.1.1 -->
- [ ] **V1.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - Evidence:
  - Remediation or N/A reason:

<!-- ASVS-REQ:V1.1.2 -->
- [ ] **V1.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.1 -->
- [ ] **V1.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.2 -->
- [ ] **V1.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.3 -->
- [ ] **V1.2.3** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.4 -->
- [ ] **V1.2.4** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.5 -->
- [ ] **V1.2.5** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.6 -->
- [ ] **V1.2.6** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.7 -->
- [ ] **V1.2.7** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.8 -->
- [ ] **V1.2.8** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.9 -->
- [ ] **V1.2.9** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.10 -->
- [ ] **V1.2.10** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V1.3.1 -->
- [ ] **V1.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.2 -->
- [ ] **V1.3.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.3 -->
- [ ] **V1.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.4 -->
- [ ] **V1.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.5 -->
- [ ] **V1.3.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.6 -->
- [ ] **V1.3.6** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.7 -->
- [ ] **V1.3.7** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.8 -->
- [ ] **V1.3.8** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.9 -->
- [ ] **V1.3.9** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.10 -->
- [ ] **V1.3.10** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.11 -->
- [ ] **V1.3.11** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.12 -->
- [ ] **V1.3.12** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V1.4.1 -->
- [ ] **V1.4.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.4.2 -->
- [ ] **V1.4.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.4.3 -->
- [ ] **V1.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.5.1 -->
- [ ] **V1.5.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.5.2 -->
- [ ] **V1.5.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.5.3 -->
- [ ] **V1.5.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V2.1.1 -->
- [ ] **V2.1.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.1.2 -->
- [ ] **V2.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.1.3 -->
- [ ] **V2.1.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.2.1 -->
- [ ] **V2.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.2.2 -->
- [ ] **V2.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.2.3 -->
- [ ] **V2.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.1 -->
- [ ] **V2.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.2 -->
- [ ] **V2.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.3 -->
- [ ] **V2.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.4 -->
- [ ] **V2.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.5 -->
- [ ] **V2.3.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V2.4.1 -->
- [ ] **V2.4.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.4.2 -->
- [ ] **V2.4.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.1.1 -->
- [ ] **V3.1.1** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3