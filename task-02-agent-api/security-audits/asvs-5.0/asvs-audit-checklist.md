# Project ASVS 5.0.0 Audit Checklist

This is the project-local working copy for the ASVS audit. Fill this file during the audit and keep it with the audited project if the project should preserve audit evidence.

## Audit Metadata

- Project: Siemens Research Agent API
- Project root: `task-02-agent-api`
- Auditor: Codex
- Created at: 2026-08-27T06:42:10Z
- ASVS version: 5.0.0
- Requirements copied: 345
- Appendix D recommendations copied: 16
- Source files embedded: 28
- Structured source files tracked: 3

## Audit Status Summary

- Requirements assessed: 345
- Pass: 68
- Fail: 0
- Partial: 28
- N/A: 218
...
- Critical findings: 0
- High findings: 0
- Moderate/local hardening findings: 0
- Residual risk: production-only ingress, TLS, IAM, managed secrets, image/SBOM,
  dependency provenance, log export/retention, storage encryption, backups, and
  DDoS controls remain Task 3/release evidence.

## Audit Scope

- Application components reviewed: FastAPI routes and exception boundary; API-key
  authentication and scopes; session, run, event, quota, worker, SQLite persistence,
  telemetry, readiness, and Task 1 executor/tool boundaries.
- Environments reviewed: local SQLite submission profile. Task 3 owns the later GCP
  deployment controls and its separate infrastructure audit.
- Authentication and authorization flows reviewed: versioned bearer API keys,
  HMAC-at-rest verification, lifecycle/rotation/revocation, route scopes, object
  ownership, worker leases, and administrative tenant deletion repositories.
- Data classes reviewed: identifiers, API-key material, user query/session labels,
  run results/events, reflections, evidence URLs, telemetry, and audit records.
- External services and third-party integrations reviewed: the imported Task 1
  guarded search/fetch/model ports. No real external request is made by this audit.
- Explicit exclusions: browser frontend, cookies, passwords, OAuth/OIDC, JWT,
  WebRTC, file upload, email, and paid or live GCP deployment. These are scored N/A
  where ASVS requires them; production-only TLS/IAM/secret/network controls remain
  Unknown or Partial until the Task 3 evidence exists.

## Remediation Log

Use this table when the audit leads to code/config/process changes. Keep the ASVS IDs traceable even after a finding is fixed.

| Date | ASVS IDs | Original status | Change or decision | Files, configs, or evidence | Verification |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 2026-08-27 | V14.3.2, V16.3.1, V16.3.2, V16.3.4 | Fail | Added `/v1` no-store responses, bounded pseudonymous auth outcomes, and a bounded correlated unexpected-error signal. | `src/agent_api/app.py`, `observability.py`, `routes/common.py`; route/event/observability tests | Full `make check`: Ruff, strict mypy, 703 tests, submission audit after staging; focused reviewer regressions: 70 passed. |

## Filling Rules

- Edit this project-local copy, not the bundled skill reference.
- For every applicable ASVS requirement, mark exactly one status: Pass, Fail, Partial, N/A, or Unknown.
- For Pass, include concrete evidence such as files, line references, tests, configs, endpoint behavior, or deployment settings.
- For Fail or Partial, describe the remediation needed. After fixing it, update the status and add a remediation-log row explaining what changed.
- For N/A, write a short reason tied to this project's architecture or scope.
- For Unknown, state what evidence is missing.
- Keep Appendix D recommendations visible, but treat them as recommended controls rather than mandatory ASVS requirements.

## Project Checklist

Every ASVS requirement in the complete checklist below has one decision, evidence,
and remediation or architecture-tied N/A/Unknown reason. Appendix D and the source
corpus remain unchanged for traceability.

## Remediated Findings

No Critical or High finding was identified in the audited local Task 2 profile.
Four defense-in-depth controls were initially Fail and are now verified Pass:

| ASVS ID | Original finding | Resolution evidence |
| :--- | :--- | :--- |
| V14.3.2 | Authenticated JSON/SSE responses lacked `Cache-Control: no-store`. | The response boundary adds `no-store` to every `/v1` JSON, error, and SSE response; route and event tests cover it. |
| V16.3.1 | Authentication success and failure were not logged. | `auth.outcome` records bounded success/failure, correlation, UTC time, and only a verified tenant pseudonym. |
| V16.3.2 | Wrong-scope denials were not logged. | The same bounded signal records `forbidden` after verified scope denial without key material or requested content. |
| V16.3.4 | Unexpected exceptions had no protected signal. | `request.unexpected_error` records UTC time and correlation only; the public response stays generic. |

The 28 Partial items have a working local subset plus a named remaining control.
The 31 Unknown items require Task 3/release artifacts that are not present in this
worktree; none is claimed as implemented.

# OWASP ASVS 5.0.0 Complete Application Security Audit Checklist

This file is generated from the local OWASP ASVS English source set.
It contains every structured ASVS requirement, Appendix D recommendations, and a verbatim source corpus from `5.0/en`.

## How to use this checklist

- Mark each ASVS requirement as Pass, Fail, Partial, N/A, or Unknown.
- Keep evidence close to the status decision: code path, config path, endpoint, test, or deployment control.
- Use N/A only with a reason tied to the application scope.
- Treat Appendix D items as recommended controls, not mandatory ASVS requirements.
- Use the source corpus section when explanatory text, references, glossary definitions, or appendices are needed.

## Coverage summary

- ASVS version: 5.0.0
- Source name: Application Security Verification Standard Project
- Chapters: 17
- Sections: 80
- Requirements: 345
- Appendix D recommendations: 16
- Embedded source files: 28
- Structured source files: 3

### Minimum level counts

| Minimum level | Requirement count |
| :--- | ---: |
| L1 | 70 |
| L2 | 183 |
| L3 | 92 |
...
| :--- | ---: | :--- |
| `en/0x00-Header.yaml` | 436 | `b8950cf9f2f248e78c52ab3b7e29d7dd531d1db4f876218e88e83d4b8b405ca2` |
| `en/0x01-Frontispiece.md` | 2369 | `92b3bc52e5db7b3c81e1c3340137d440f1bd795c1ed018f9d64c996976b8f63f` |
| `en/0x02-Preface.md` | 2786 | `fe6ac41f02478b23adbc2ec03bb9f2f8a9aafef5e6fcf7518892240869811dde` |
| `en/0x03-What-is-the-ASVS.md` | 21019 | `dd1fc99f48b5cf1974bcbdca0266ab7fea9e1bdf6cb21a811114514413f7ba53` |
| `en/0x04-Assessment_and_Certification.md` | 5399 | `76e12b7363cbfb6b4c8f90c8e455a040b14f9e7e12d6a1025f022db39b1d9a0c` |
| `en/0x05-For-Users-Of-4.0.md` | 8580 | `caaa6b7046b409aba5522e1aca948d2b80aa09fc08add66f046eeeb8b8f5fb79` |
| `en/0x10-V1-Encoding-and-Sanitization.md` | 12968 | `8c0e36a42c3cbf711e2579eb68e1fea8090f52301cac7c400d7c392207b24ee7` |
| `en/0x11-V2-Validation-and-Business-Logic.md` | 7100 | `70ad0f68df22ddd825c2837e74999fa026851e75a3128372307685e2bb526969` |
| `en/0x12-V3-Web-Frontend-Security.md` | 13546 | `02de197d5aa55592cfa524e66d4aa15bd3e8955e23e76438ac413c94172488fc` |
| `en/0x13-V4-API-and-Web-Service.md` | 7087 | `efcadbf5ad78fb977c96d3d30e9a1d02aaafd4d787112a529ac82788a990406b` |
| `en/0x14-V5-File-Handling.md` | 5101 | `079a51123e5156a5ffc6329f266d49752cb60f6b780899484c3f1cfde3734ebb` |
| `en/0x15-V6-Authentication.md` | 22549 | `e1e0aaa15e48f7941560176d28eeb11ad3b9b4f9d323437c0e950b3a16cd2b23` |
| `en/0x16-V7-Session-Management.md` | 8989 | `4aec329f2642ae750fbe1abac5da7e8a7bcfec6fc331bb0a46916970f9c88236` |
| `en/0x17-V8-Authorization.md` | 5920 | `60c188b1703cab2086841d8f1b4adc0ededf66ed38c676a11bb92347646b5330` |
| `en/0x18-V9-Self-contained-Tokens.md` | 4130 | `5f14e09fb67892e805ad386959eb3a8bafca03271dbb502ccf7323e82b3f812c` |
| `en/0x19-V10-OAuth-and-OIDC.md` | 20669 | `2da443986ba40987bd0908bbd7da2372ab0d777701cc590f4eb0fdad6ae804ed` |
| `en/0x20-V11-Cryptography.md` | 11998 | `a64f3f2dc6f53565dfba66e40fd336b50eab6cdb530c2791f62db8b58202bacf` |
| `en/0x21-V12-Secure-Communication.md` | 5233 | `6c8407229aa12349b8c13e4681b12e25cac54c41a63a661617003039abf8ae4a` |
| `en/0x22-V13-Configuration.md` | 7828 | `f66ef1306eba07e04fcd0f23b52d082747f1b4fba0a19934167fb35507afba6d` |
| `en/0x23-V14-Data-Protection.md` | 7285 | `a9ba33b7c77379fed0f27fd019945e10fa143c63b8c4e4783b7cf587c9f56039` |
| `en/0x24-V15-Secure-Coding-and-Architecture.md` | 10222 | `4c068e82b157159e9ab7099daecb40af4c2642ad791751f791a26785dead6b22` |
| `en/0x25-V16-Security-Logging-and-Error-Handling.md` | 8174 | `23b0c0d4a54cc62a53e8669cbd15bb97f323249e9d837d3055f95873b2ef1652` |
| `en/0x26-V17-WebRTC.md` | 8738 | `29a55aed304efc92f6bf827aeee3323671d0521ddab823f8e6265018ce0ff2c0` |
| `en/0x90-Appendix-A_Glossary.md` | 18406 | `ee9bc08847c912bf736a04b76a4e36f0ee62d20ec4280dfe8d4852f6e51a8e74` |
| `en/0x91-Appendix-B_References.md` | 2423 | `26035080eb19d0328edc92b5f8b72dd6b914a135c839932e89c92e12f8e65411` |
| `en/0x92-Appendix-C_Cryptography.md` | 23581 | `3d96d69719d9beebcae41a02d6876ed1c98323380a04609f0d50f6a175eadde1` |
| `en/0x93-Appendix-D_Recommendations.md` | 4955 | `e7992618aecf8561f692187344b17704e5a4639bf6a692417e9a719791c83ba1` |
| `en/0x94-Appendix-E_Contributors.md` | 14702 | `0e3768fb36298aede261c51eea04fe67a93f44ac3be7bc92e21582cc1a682819` |

### Structured source files

| Source file | Bytes | SHA-256 |
| :--- | ---: | :--- |
| `docs_en/OWASP_Application_Security_Verification_Standard_5.0.0_en.json` | 149407 | `bcdbec214d70abcfad9284a31d4f9e5134305831d628aad3aa85d7e26626cb35` |
| `docs_en/OWASP_Application_Security_Verification_Standard_5.0.0_en.flat.json` | 162870 | `8201b20eec2908c3380ac600c91c8ba746346fbb808859366abb232027532311` |
| `docs_en/OWASP_Application_Security_Verification_Standard_5.0.0_en.flat.legacy.json` | 192129 | `d9cd7701b1e307636c08bd0f865e5bdab784548a29e1683edc847d7b6b23be0f` |

## Requirements checklist

Each requirement item includes blank fields for audit status, evidence, and remediation.
...
<!-- ASVS-REQ:V1.1.1 -->
- [x] **V1.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - CWE: -
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: FastAPI/Pydantic canonicalize strict JSON once; framework JSON/SSE encoding, parameter-bound SQLite, bounded types, Python memory safety, and context-managed resources are verified by schema, attack-surface, storage, limit, and lifecycle tests.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V1.1.2 -->
- [x] **V1.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.1 -->
- [x] **V1.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.2 -->
- [x] **V1.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
  ...
  - CWE: -
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: JSON/SSE Task 2 has no HTML, SVG, template, LDAP, XPath, LaTeX, CSV, mail, memcache, XML, dynamic-regex, or OS-command surface described here.
  - Remediation or N/A reason: Not applicable: JSON/SSE Task 2 has no HTML, SVG, template, LDAP, XPath, LaTeX, CSV, mail, memcache, XML, dynamic-regex, or OS-command surface described here.

<!-- ASVS-REQ:V1.2.3 -->
- [x] **V1.2.3** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.4 -->
- [x] **V1.2.4** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.5 -->
- [x] **V1.2.5** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.6 -->
- [x] **V1.2.6** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.7 -->
- [x] **V1.2.7** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.8 -->
- [x] **V1.2.8** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.9 -->
- [x] **V1.2.9** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.2.10 -->
- [x] **V1.2.10** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V1.3.1 -->
- [x] **V1.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.2 -->
- [x] **V1.3.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.3 -->
- [x] **V1.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.4 -->
- [x] **V1.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.5 -->
- [x] **V1.3.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.6 -->
- [x] **V1.3.6** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - CWE: -
  - NIST: -
  - Status: [ ] Pass [ ] Fail [x] Partial [ ] N/A [ ] Unknown
  - Evidence: Task 2 performs no direct fetch and documents the reviewed Task 1 URL guard, but create_app accepts an arbitrary injected RunExecutor.
  - Remediation or N/A reason: Deploy only the reviewed Task 1 executor or prove equivalent protocol, host, DNS, port, redirect, content, and size SSRF controls.

<!-- ASVS-REQ:V1.3.7 -->
- [x] **V1.3.7** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.8 -->
- [x] **V1.3.8** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.9 -->
- [x] **V1.3.9** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.10 -->
- [x] **V1.3.10** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.11 -->
- [x] **V1.3.11** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.3.12 -->
- [x] **V1.3.12** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V1.4.1 -->
- [x] **V1.4.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.4.2 -->
- [x] **V1.4.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.4.3 -->
- [x] **V1.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.5.1 -->
- [x] **V1.5.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.5.2 -->
- [x] **V1.5.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V1.5.3 -->
- [x] **V1.5.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V2.1.1 -->
- [x] **V2.1.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: The checked OpenAPI contract, strict schemas, server-side dependencies, transactional services, versions, leases, idempotency, and quotas are verified by route, service, repository, cancellation, and worker tests.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V2.1.2 -->
- [x] **V2.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.1.3 -->
- [x] **V2.1.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.2.1 -->
- [x] **V2.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.2.2 -->
- [x] **V2.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.2.3 -->
- [x] **V2.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.1 -->
- [x] **V2.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.2 -->
- [x] **V2.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.3 -->
- [x] **V2.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.4 -->
- [x] **V2.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.3.5 -->
- [x] **V2.3.5** (minimum level: L3; L1: no; L2: no; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 has no human-timed or multi-person high-value approval flow; it is an asynchronous machine API.
  - Remediation or N/A reason: Not applicable: Task 2 has no human-timed or multi-person high-value approval flow; it is an asynchronous machine API.

#### V2.4 Anti-automation

<!-- ASVS-REQ:V2.4.1 -->
- [x] **V2.4.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V2.4.2 -->
- [x] **V2.4.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
Requirement count: 31

#### V3.1 Web Frontend Security Documentation

<!-- ASVS-REQ:V3.1.1 -->
- [x] **V3.1.1** (minimum level: L3; L1: no; L2: no; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 has no browser frontend, cookies, DOM, client assets, postMessage, UI redirects, or browser origin.
  - Remediation or N/A reason: Not applicable: Task 2 has no browser frontend, cookies, DOM, client assets, postMessage, UI redirects, or browser origin.

#### V3.2 Unintended Content Interpretation

<!-- ASVS-REQ:V3.2.1 -->
- [x] **V3.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.2.2 -->
- [x] **V3.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.2.3 -->
- [x] **V3.2.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.3.1 -->
- [x] **V3.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.3.2 -->
- [x] **V3.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.3.3 -->
- [x] **V3.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.3.4 -->
- [x] **V3.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.3.5 -->
- [x] **V3.3.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.4.1 -->
- [x] **V3.4.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.4.2 -->
- [x] **V3.4.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.4.3 -->
- [x] **V3.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.4.4 -->
- [x] **V3.4.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.4.5 -->
- [x] **V3.4.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.4.6 -->
- [x] **V3.4.6** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.4.7 -->
- [x] **V3.4.7** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.4.8 -->
- [x] **V3.4.8** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.5.1 -->
- [x] **V3.5.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.5.2 -->
- [x] **V3.5.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.5.3 -->
- [x] **V3.5.3** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.5.4 -->
- [x] **V3.5.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.5.5 -->
- [x] **V3.5.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.5.6 -->
- [x] **V3.5.6** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.5.7 -->
- [x] **V3.5.7** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.5.8 -->
- [x] **V3.5.8** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.6.1 -->
- [x] **V3.6.1** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.7.1 -->
- [x] **V3.7.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.7.2 -->
- [x] **V3.7.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V3.7.3 -->
- [x] **V3.7.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.7.4 -->
- [x] **V3.7.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V3.7.5 -->
- [x] **V3.7.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V4.1.1 -->
- [x] **V4.1.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: FastAPI/Starlette set the tested JSON/SSE content types and reject undeclared methods with the generic 405 envelope; route and SSE contract tests assert both.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V4.1.2 -->
- [x] **V4.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 has no HTTP redirect, trusted proxy-identity header, signed high-value transaction, outbound request builder, GraphQL, or WebSocket surface described here.
  - Remediation or N/A reason: Not applicable: Task 2 has no HTTP redirect, trusted proxy-identity header, signed high-value transaction, outbound request builder, GraphQL, or WebSocket surface described here.

<!-- ASVS-REQ:V4.1.3 -->
- [x] **V4.1.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V4.1.4 -->
- [x] **V4.1.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V4.1.5 -->
- [x] **V4.1.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V4.2.1 -->
- [x] **V4.2.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - CWE: -
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [ ] N/A [x] Unknown
  - Evidence: No production HTTP server, proxy, or ingress configuration exists to verify end-to-end framing, header parsing, or request-smuggling defenses.
  - Remediation or N/A reason: Task 3/release must supply and verify the missing production evidence before this item can be scored.

<!-- ASVS-REQ:V4.2.2 -->
- [x] **V4.2.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V4.2.3 -->
- [x] **V4.2.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V4.2.4 -->
- [x] **V4.2.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V4.2.5 -->
- [x] **V4.2.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V4.3.1 -->
- [x] **V4.3.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V4.3.2 -->
- [x] **V4.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V4.4.1 -->
- [x] **V4.4.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V4.4.2 -->
- [x] **V4.4.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V4.4.3 -->
- [x] **V4.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V4.4.4 -->
- [x] **V4.4.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V5.1.1 -->
- [x] **V5.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 has no upload, download, archive, user-controlled path, media processor, or file-serving endpoint.
  - Remediation or N/A reason: Not applicable: Task 2 has no upload, download, archive, user-controlled path, media processor, or file-serving endpoint.

#### V5.2 File Upload and Content

<!-- ASVS-REQ:V5.2.1 -->
- [x] **V5.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V5.2.2 -->
- [x] **V5.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V5.2.3 -->
- [x] **V5.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V5.2.4 -->
- [x] **V5.2.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V5.2.5 -->
- [x] **V5.2.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V5.2.6 -->
- [x] **V5.2.6** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V5.3.1 -->
- [x] **V5.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V5.3.2 -->
- [x] **V5.3.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V5.3.3 -->
- [x] **V5.3.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V5.4.1 -->
- [x] **V5.4.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V5.4.2 -->
- [x] **V5.4.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V5.4.3 -->
- [x] **V5.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.1.1 -->
- [x] **V6.1.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 uses opaque service API keys and has no end-user, password, MFA, recovery, OTP, biometric, SAML, or identity-provider flow.
  - Remediation or N/A reason: Not applicable: Task 2 uses opaque service API keys and has no end-user, password, MFA, recovery, OTP, biometric, SAML, or identity-provider flow.

<!-- ASVS-REQ:V6.1.2 -->
- [x] **V6.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.1.3 -->
- [x] **V6.1.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.1 -->
- [x] **V6.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.2 -->
- [x] **V6.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.3 -->
- [x] **V6.2.3** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.4 -->
- [x] **V6.2.4** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.5 -->
- [x] **V6.2.5** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.6 -->
- [x] **V6.2.6** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.7 -->
- [x] **V6.2.7** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.8 -->
- [x] **V6.2.8** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.9 -->
- [x] **V6.2.9** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.10 -->
- [x] **V6.2.10** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.11 -->
- [x] **V6.2.11** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.2.12 -->
- [x] **V6.2.12** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.3.1 -->
- [x] **V6.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.3.2 -->
- [x] **V6.3.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.3.3 -->
- [x] **V6.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.3.4 -->
- [x] **V6.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.3.5 -->
- [x] **V6.3.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.3.6 -->
- [x] **V6.3.6** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.3.7 -->
- [x] **V6.3.7** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.3.8 -->
- [x] **V6.3.8** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.4.1 -->
- [x] **V6.4.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.4.2 -->
- [x] **V6.4.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.4.3 -->
- [x] **V6.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.4.4 -->
- [x] **V6.4.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.4.5 -->
- [x] **V6.4.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.4.6 -->
- [x] **V6.4.6** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.5.1 -->
- [x] **V6.5.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.5.2 -->
- [x] **V6.5.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.5.3 -->
- [x] **V6.5.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.5.4 -->
- [x] **V6.5.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.5.5 -->
- [x] **V6.5.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.5.6 -->
- [x] **V6.5.6** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.5.7 -->
- [x] **V6.5.7** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.5.8 -->
- [x] **V6.5.8** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.6.1 -->
- [x] **V6.6.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.6.2 -->
- [x] **V6.6.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.6.3 -->
- [x] **V6.6.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.6.4 -->
- [x] **V6.6.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.7.1 -->
- [x] **V6.7.1** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.7.2 -->
- [x] **V6.7.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V6.8.1 -->
- [x] **V6.8.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.8.2 -->
- [x] **V6.8.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.8.3 -->
- [x] **V6.8.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V6.8.4 -->
- [x] **V6.8.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.1.1 -->
- [x] **V7.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 creates no login session, cookie, CSRF state, or session token; every request revalidates its opaque API key.
  - Remediation or N/A reason: Not applicable: Task 2 creates no login session, cookie, CSRF state, or session token; every request revalidates its opaque API key.

<!-- ASVS-REQ:V7.1.2 -->
- [x] **V7.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.1.3 -->
- [x] **V7.1.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.2.1 -->
- [x] **V7.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.2.2 -->
- [x] **V7.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.2.3 -->
- [x] **V7.2.3** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.2.4 -->
- [x] **V7.2.4** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.3.1 -->
- [x] **V7.3.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.3.2 -->
- [x] **V7.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.4.1 -->
- [x] **V7.4.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.4.2 -->
- [x] **V7.4.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.4.3 -->
- [x] **V7.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.4.4 -->
- [x] **V7.4.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.4.5 -->
- [x] **V7.4.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.5.1 -->
- [x] **V7.5.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.5.2 -->
- [x] **V7.5.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.5.3 -->
- [x] **V7.5.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V7.6.1 -->
- [x] **V7.6.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V7.6.2 -->
- [x] **V7.6.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V8.1.1 -->
- [x] **V8.1.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: Scopes are enforced in trusted route dependencies; tenant identity is key-derived and repeated in repository predicates; strict projections and cross-tenant, wrong-scope, and mass-assignment tests cover the boundary.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V8.1.2 -->
- [x] **V8.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V8.1.3 -->
- [x] **V8.1.3** (minimum level: L3; L1: no; L2: no; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: The local authorization model uses no environmental/adaptive factor or downstream intermediary identity.
  - Remediation or N/A reason: Not applicable: The local authorization model uses no environmental/adaptive factor or downstream intermediary identity.

<!-- ASVS-REQ:V8.1.4 -->
- [x] **V8.1.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V8.2.1 -->
- [x] **V8.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V8.2.2 -->
- [x] **V8.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V8.2.3 -->
- [x] **V8.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V8.2.4 -->
- [x] **V8.2.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V8.3.1 -->
- [x] **V8.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V8.3.2 -->
- [x] **V8.3.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V8.3.3 -->
- [x] **V8.3.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V8.4.1 -->
- [x] **V8.4.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V8.4.2 -->
- [x] **V8.4.2** (minimum level: L3; L1: no; L2: no; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [x] Partial [ ] N/A [ ] Unknown
  - Evidence: No public admin route exists and key administration is a local CLI, but operator identity, device, and IAM controls are outside this worktree.
  - Remediation or N/A reason: Task 3 must prove operator identity, least-privilege IAM, device/access policy, and auditable privileged execution.
...
<!-- ASVS-REQ:V9.1.1 -->
- [x] **V9.1.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 issues and accepts no self-contained token or JWT; opaque keys are looked up on every request.
  - Remediation or N/A reason: Not applicable: Task 2 issues and accepts no self-contained token or JWT; opaque keys are looked up on every request.

<!-- ASVS-REQ:V9.1.2 -->
- [x] **V9.1.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V9.1.3 -->
- [x] **V9.1.3** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V9.2.1 -->
- [x] **V9.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V9.2.2 -->
- [x] **V9.2.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V9.2.3 -->
- [x] **V9.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V9.2.4 -->
- [x] **V9.2.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.1.1 -->
- [x] **V10.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 has no OAuth/OIDC role, grant, client, token, PAR, JARM, or userinfo endpoint.
  - Remediation or N/A reason: Not applicable: Task 2 has no OAuth/OIDC role, grant, client, token, PAR, JARM, or userinfo endpoint.

<!-- ASVS-REQ:V10.1.2 -->
- [x] **V10.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.2.1 -->
- [x] **V10.2.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.2.2 -->
- [x] **V10.2.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.2.3 -->
- [x] **V10.2.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V10.3.1 -->
- [x] **V10.3.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.3.2 -->
- [x] **V10.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.3.3 -->
- [x] **V10.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.3.4 -->
- [x] **V10.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.3.5 -->
- [x] **V10.3.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V10.4.1 -->
- [x] **V10.4.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.2 -->
- [x] **V10.4.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.3 -->
- [x] **V10.4.3** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.4 -->
- [x] **V10.4.4** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.5 -->
- [x] **V10.4.5** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.6 -->
- [x] **V10.4.6** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.7 -->
- [x] **V10.4.7** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.8 -->
- [x] **V10.4.8** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.9 -->
- [x] **V10.4.9** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.10 -->
- [x] **V10.4.10** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.11 -->
- [x] **V10.4.11** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.4.12 -->
- [x] **V10.4.12** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V10.4.13 -->
- [x] **V10.4.13** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V10.4.14 -->
- [x] **V10.4.14** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V10.4.15 -->
- [x] **V10.4.15** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V10.4.16 -->
- [x] **V10.4.16** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V10.5.1 -->
- [x] **V10.5.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.5.2 -->
- [x] **V10.5.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.5.3 -->
- [x] **V10.5.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.5.4 -->
- [x] **V10.5.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.5.5 -->
- [x] **V10.5.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.6.1 -->
- [x] **V10.6.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.6.2 -->
- [x] **V10.6.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.7.1 -->
- [x] **V10.7.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.7.2 -->
- [x] **V10.7.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V10.7.3 -->
- [x] **V10.7.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.1.1 -->
- [x] **V11.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [x] Partial [ ] N/A [ ] Unknown
  - Evidence: Key lifecycle and HMAC usage exist and are documented, but managed key policy/inventory/agility, memory/storage encryption, and rotation evidence are incomplete.
  - Remediation or N/A reason: Task 3/release must complete key inventory/policy, rotation and migration, managed storage, process/storage protection, and lifecycle evidence.

<!-- ASVS-REQ:V11.1.2 -->
- [x] **V11.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.1.3 -->
- [x] **V11.1.3** (minimum level: L3; L1: no; L2: no; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [ ] N/A [x] Unknown
  - Evidence: No maintained cryptographic discovery/PQC plan or host memory-encryption evidence exists beyond the reviewed local HMAC code.
  - Remediation or N/A reason: Task 3/release must supply and verify the missing production evidence before this item can be scored.

<!-- ASVS-REQ:V11.1.4 -->
- [x] **V11.1.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V11.2.1 -->
- [x] **V11.2.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: Python standard-library secrets and domain-separated HMAC-SHA-256 provide 32-byte key material, 256-bit digests, and hmac.compare_digest; auth and observability tests cover weak secrets, lifecycle, isolation, and oracle-safe failures.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V11.2.2 -->
- [x] **V11.2.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.2.3 -->
- [x] **V11.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.2.4 -->
- [x] **V11.2.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V11.2.5 -->
- [x] **V11.2.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V11.3.1 -->
- [x] **V11.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
  - Requirement: Verify that insecure block modes (e.g., ECB) and weak padding schemes (e.g., PKCS#1 v1.5) are not used.
  - CWE: -
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Local cryptography is HMAC/CSPRNG only; there is no password hash, cipher mode, password-derived key, asymmetric key, signature, or key exchange here.
  - Remediation or N/A reason: Not applicable: Local cryptography is HMAC/CSPRNG only; there is no password hash, cipher mode, password-derived key, asymmetric key, signature, or key exchange here.

<!-- ASVS-REQ:V11.3.2 -->
- [x] **V11.3.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.3.3 -->
- [x] **V11.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.3.4 -->
- [x] **V11.3.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V11.3.5 -->
- [x] **V11.3.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V11.4.1 -->
- [x] **V11.4.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.4.2 -->
- [x] **V11.4.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.4.3 -->
- [x] **V11.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.4.4 -->
- [x] **V11.4.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.5.1 -->
- [x] **V11.5.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.5.2 -->
- [x] **V11.5.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V11.6.1 -->
- [x] **V11.6.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V11.6.2 -->
- [x] **V11.6.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V11.7.1 -->
- [x] **V11.7.1** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V11.7.2 -->
- [x] **V11.7.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V12.1.1 -->
- [x] **V12.1.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [ ] N/A [x] Unknown
  - Evidence: No production TLS listener, certificate, protocol/cipher, ECH/OCSP, or transport-policy evidence exists in Task 2.
  - Remediation or N/A reason: Task 3/release must supply and verify the missing production evidence before this item can be scored.

<!-- ASVS-REQ:V12.1.2 -->
- [x] **V12.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V12.1.3 -->
- [x] **V12.1.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: The local monolith/SQLite profile has no mTLS identity, outbound TLS client, or networked internal service link described here.
  - Remediation or N/A reason: Not applicable: The local monolith/SQLite profile has no mTLS identity, outbound TLS client, or networked internal service link described here.

<!-- ASVS-REQ:V12.1.4 -->
- [x] **V12.1.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V12.1.5 -->
- [x] **V12.1.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V12.2.1 -->
- [x] **V12.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V12.2.2 -->
- [x] **V12.2.2** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V12.3.1 -->
- [x] **V12.3.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V12.3.2 -->
- [x] **V12.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V12.3.3 -->
- [x] **V12.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V12.3.4 -->
- [x] **V12.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V12.3.5 -->
- [x] **V12.3.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V13.1.1 -->
- [x] **V13.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: docs/api.md, docs/operations.md, and docs/threat-model.md inventory local/injected communication, concurrency, resource release, timeout, retry, lease, and failure behavior; limit and lifecycle tests verify those decisions.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V13.1.2 -->
- [x] **V13.1.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V13.1.3 -->
- [x] **V13.1.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V13.1.4 -->
- [x] **V13.1.4** (minimum level: L3; L1: no; L2: no; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [x] Partial [ ] N/A [ ] Unknown
  - Evidence: Local configuration fails closed and service budgets are documented, but executor selection, managed secrets, service connection policy, runtime flags, and production surface controls are incomplete.
  - Remediation or N/A reason: Task 3 must implement and verify the missing managed-secret, service identity/egress, connection, runtime, and ingress controls for this requirement.

#### V13.2 Backend Communication Configuration

<!-- ASVS-REQ:V13.2.1 -->
- [x] **V13.2.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [ ] N/A [x] Unknown
  - Evidence: No Task 3 deployment IAM, service identity, egress, secret-module, container, or web-tier configuration exists.
  - Remediation or N/A reason: Task 3/release must supply and verify the missing production evidence before this item can be scored.

<!-- ASVS-REQ:V13.2.2 -->
- [x] **V13.2.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.2.3 -->
- [x] **V13.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: The local application has no default backend service credential; production service credentials belong to Task 3.
  - Remediation or N/A reason: Not applicable: The local application has no default backend service credential; production service credentials belong to Task 3.

<!-- ASVS-REQ:V13.2.4 -->
- [x] **V13.2.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.2.5 -->
- [x] **V13.2.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.2.6 -->
- [x] **V13.2.6** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V13.3.1 -->
- [x] **V13.3.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.3.2 -->
- [x] **V13.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.3.3 -->
- [x] **V13.3.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V13.3.4 -->
- [x] **V13.3.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V13.4.1 -->
- [x] **V13.4.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.4.2 -->
- [x] **V13.4.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.4.3 -->
- [x] **V13.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.4.4 -->
- [x] **V13.4.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.4.5 -->
- [x] **V13.4.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V13.4.6 -->
- [x] **V13.4.6** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V13.4.7 -->
- [x] **V13.4.7** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V14.1.1 -->
- [x] **V14.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: docs/threat-model.md classifies protected data; credentials are header-only, queries body-only, no trackers exist, and explicit response/telemetry models omit secrets and internals as verified by route, attack-surface, and redaction tests.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V14.1.2 -->
- [x] **V14.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [x] Partial [ ] N/A [ ] Unknown
  - Evidence: Local minimization, access, deletion, and redaction controls exist, but encryption, cache, retention, backup, and exported-log requirements are incomplete.
  - Remediation or N/A reason: Task 3/release must complete data-class encryption, retention/deletion, cache, backup, and log-access controls required by this item.

#### V14.2 General Data Protection

<!-- ASVS-REQ:V14.2.1 -->
- [x] **V14.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V14.2.2 -->
- [x] **V14.2.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V14.2.3 -->
- [x] **V14.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V14.2.4 -->
- [x] **V14.2.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V14.2.5 -->
- [x] **V14.2.5** (minimum level: L3; L1: no; L2: no; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [ ] N/A [x] Unknown
  - Evidence: No production CDN, load-balancer, or cache configuration exists to verify content-type-aware cache behavior.
  - Remediation or N/A reason: Task 3/release must supply and verify the missing production evidence before this item can be scored.

<!-- ASVS-REQ:V14.2.6 -->
- [x] **V14.2.6** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V14.2.7 -->
- [x] **V14.2.7** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V14.2.8 -->
- [x] **V14.2.8** (minimum level: L3; L1: no; L2: no; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 has no uploaded-file metadata or browser storage/DOM lifecycle.
  - Remediation or N/A reason: Not applicable: Task 2 has no uploaded-file metadata or browser storage/DOM lifecycle.

#### V14.3 Client-side Data Protection

<!-- ASVS-REQ:V14.3.1 -->
- [x] **V14.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V14.3.2 -->
- [x] **V14.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: The response boundary sets Cache-Control: no-store on every /v1 JSON, error, and SSE response; test_observability.py and routes/test_events.py verify success, denial, catch-all, and stream responses.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V14.3.3 -->
- [x] **V14.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.1.1 -->
- [x] **V15.1.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [ ] N/A [x] Unknown
  - Evidence: No remediation SLA, SBOM, vulnerability/provenance report, production image inventory, or trusted-proxy configuration exists.
  - Remediation or N/A reason: Task 3/release must supply and verify the missing production evidence before this item can be scored.

<!-- ASVS-REQ:V15.1.2 -->
- [x] **V15.1.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.1.3 -->
- [x] **V15.1.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: The threat model identifies expensive/dangerous boundaries; quotas, durable queues, leases, strict models/projections, transactions, localized locks, and readiness identity checks are verified by quota, worker, attack-surface, and race tests.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V15.1.4 -->
- [x] **V15.1.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V15.1.5 -->
- [x] **V15.1.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V15.2.1 -->
- [x] **V15.2.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.2.2 -->
- [x] **V15.2.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.2.3 -->
- [x] **V15.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.2.4 -->
- [x] **V15.2.4** (minimum level: L3; L1: no; L2: no; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [x] Partial [ ] N/A [ ] Unknown
  - Evidence: Local locking, typed executor boundaries, dependency declarations, and redirect ownership exist, but provenance, isolation, duplicate-query policy, and fairness evidence are incomplete.
  - Remediation or N/A reason: Task 3/release must complete provenance, isolation, redirect binding, duplicate-parameter policy, and fairness evidence as applicable to this item.

<!-- ASVS-REQ:V15.2.5 -->
- [x] **V15.2.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V15.3.1 -->
- [x] **V15.3.1** (minimum level: L1; L1: yes; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.3.2 -->
- [x] **V15.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.3.3 -->
- [x] **V15.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.3.4 -->
- [x] **V15.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.3.5 -->
- [x] **V15.3.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.3.6 -->
- [x] **V15.3.6** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 contains no JavaScript or prototype object model.
  - Remediation or N/A reason: Not applicable: Task 2 contains no JavaScript or prototype object model.

<!-- ASVS-REQ:V15.3.7 -->
- [x] **V15.3.7** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V15.4.1 -->
- [x] **V15.4.1** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V15.4.2 -->
- [x] **V15.4.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V15.4.3 -->
- [x] **V15.4.3** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V15.4.4 -->
- [x] **V15.4.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V16.1.1 -->
- [x] **V16.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [x] Partial [ ] N/A [ ] Unknown
  - Evidence: Bounded authentication, authorization-denial, and unexpected-error signals now complement application logging, audit, redaction, and the safe catch-all; production inventory, protection, retention, export, and alerting remain Task 3 evidence.
  - Remediation or N/A reason: Task 3 must prove log destinations, access, immutability, retention, export, and alerting.

#### V16.2 General Logging

<!-- ASVS-REQ:V16.2.1 -->
- [x] **V16.2.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: Typed one-line JSON telemetry uses UTC, fixed fields, pseudonyms, JSON encoding, and redaction; generic correlated errors and fail-closed service/storage/worker paths are verified by observability and fault tests.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V16.2.2 -->
- [x] **V16.2.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.2.3 -->
- [x] **V16.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.2.4 -->
- [x] **V16.2.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.2.5 -->
- [x] **V16.2.5** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.3.1 -->
- [x] **V16.3.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: authenticate_request emits bounded authenticated and unauthenticated outcomes with UTC time and correlation; only verified identities receive an HMAC tenant pseudonym. test_run_routes_emit_joinable_safe_logs_and_idempotent_audit verifies success, failure, metrics, and secret exclusion.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V16.3.2 -->
- [x] **V16.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: A verified wrong-scope decision emits the bounded forbidden outcome, UTC time, correlation, and HMAC tenant pseudonym without the key, request body, scope string, or attacker input; the observability route test verifies it.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

<!-- ASVS-REQ:V16.3.3 -->
- [x] **V16.3.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.3.4 -->
- [x] **V16.3.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [x] Pass [ ] Fail [ ] Partial [ ] N/A [ ] Unknown
  - Evidence: _internal_error emits request.unexpected_error with UTC time and correlation only, increments a no-label counter, and retains the fixed generic envelope; test_unexpected_errors_emit_a_safe_signal_and_private_response verifies no exception leakage.
  - Remediation or N/A reason: None for the audited local Task 2 scope.

#### V16.4 Log Protection

<!-- ASVS-REQ:V16.4.1 -->
- [x] **V16.4.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.4.2 -->
- [x] **V16.4.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.4.3 -->
- [x] **V16.4.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [ ] N/A [x] Unknown
  - Evidence: No separate production log sink/export transport, access policy, alerting, or escalation evidence exists.
  - Remediation or N/A reason: Task 3/release must supply and verify the missing production evidence before this item can be scored.

#### V16.5 Error Handling

<!-- ASVS-REQ:V16.5.1 -->
- [x] **V16.5.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.5.2 -->
- [x] **V16.5.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.5.3 -->
- [x] **V16.5.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V16.5.4 -->
- [x] **V16.5.4** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V17.1.1 -->
- [x] **V17.1.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
  ...
  - NIST: -
  - Status: [ ] Pass [ ] Fail [ ] Partial [x] N/A [ ] Unknown
  - Evidence: Task 2 has no WebRTC, TURN, DTLS, SRTP, media server, recording, or signaling component.
  - Remediation or N/A reason: Not applicable: Task 2 has no WebRTC, TURN, DTLS, SRTP, media server, recording, or signaling component.

<!-- ASVS-REQ:V17.1.2 -->
- [x] **V17.1.2** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V17.2.1 -->
- [x] **V17.2.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V17.2.2 -->
- [x] **V17.2.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V17.2.3 -->
- [x] **V17.2.3** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V17.2.4 -->
- [x] **V17.2.4** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V17.2.5 -->
- [x] **V17.2.5** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V17.2.6 -->
- [x] **V17.2.6** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V17.2.7 -->
- [x] **V17.2.7** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V17.2.8 -->
- [x] **V17.2.8** (minimum level: L3; L1: no; L2: no; L3: yes)
...
<!-- ASVS-REQ:V17.3.1 -->
- [x] **V17.3.1** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
<!-- ASVS-REQ:V17.3.2 -->
- [x] **V17.3.2** (minimum level: L2; L1: no; L2: yes; L3: yes)
...
## Appendix D recommendations checklist

- [ ] **REC-D-001** (Recommended, in-scope mechanisms)
  - Recommendation: A password strength meter should provided to help users set a stronger password.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-002** (Recommended, in-scope mechanisms)
  - Recommendation: Create a publicly available security.txt file at the root or .well-known directory of the application that clearly defines a link or e-mail address for people to contact owners about security issues.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-003** (Recommended, in-scope mechanisms)
  - Recommendation: Client-side input validation should be enforced in addition to validation at a trusted service layer as this provides a good opportunity to discover when someone has bypassed client-side controls in an attempt to attack the application.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-004** (Recommended, in-scope mechanisms)
  - Recommendation: Prevent accidentally accessible and sensitive pages from appearing in search engines using a robots.txt file, the X-Robots-Tag response header or a robots html meta tag.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-005** (Recommended, in-scope mechanisms)
  - Recommendation: When using GraphQL, implement authorization logic at the business logic layer instead of the GraphQL or resolver layer to avoid having to handle authorization on every separate interface.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-006** (Software Security principles)
  - Recommendation: Security controls should be centralized, simple (economy of design), verifiably secure, and reusable. This should avoid duplicate, missing, or ineffective controls.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-007** (Software Security principles)
  - Recommendation: Wherever possible, use previously written and well-vetted security control implementations rather than relying on implementing controls from scratch.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-008** (Software Security principles)
  - Recommendation: Ideally, a single access control mechanism should be used to access protected data and resources. All requests should pass through this single mechanism to avoid copy and paste or insecure alternative paths.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-009** (Software Security principles)
  - Recommendation: Attribute or feature-based access control is a recommended pattern whereby the code checks the user's authorization for a feature or data item rather than just their role. Permissions should still be allocated using roles.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-010** (Software Security processes)
  - Recommendation: Verify the use of a secure software development lifecycle that addresses security in all stages of development.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-011** (Software Security processes)
  - Recommendation: Verify the use of threat modeling for every design change or sprint planning to identify threats, plan for countermeasures, facilitate appropriate risk responses, and guide security testing.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-012** (Software Security processes)
  - Recommendation: Verify that all user stories and features contain functional security constraints, such as "As a user, I should be able to view and edit my profile. I should not be able to view or edit anyone else's profile"
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-013** (Software Security processes)
  - Recommendation: Verify availability of a secure coding checklist, security requirements, guideline, or policy to all developers and testers.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-014** (Software Security processes)
  - Recommendation: Verify that an ongoing process exists to ensure that the application source code is free from backdoors, malicious code (e.g., salami attacks, logic bombs, time bombs), and undocumented or hidden features (e.g., Easter eggs, insecure debugging tools). Complying with this section is not possible without complete access to source code, including third-party libraries, and is therefore probably only suitable for applications requiring the very highest levels of security.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-015** (Software Security processes)
  - Recommendation: Verify that mechanisms are in place to detect and respond to configuration drift in deployed environments. This may include using immutable infrastructure, automated redeployment from a secure baseline, or drift detection tools that compare current state against approved configurations.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

- [ ] **REC-D-016** (Software Security processes)
  - Recommendation: Verify that configuration hardening is performed on all third-party products, libraries, frameworks, and services as per their individual recommendations.
  - Status: [ ] Adopted [ ] Deferred [ ] N/A [ ] Unknown
  - Evidence or decision note:

## Source corpus

The following files are embedded verbatim from the ASVS English source directory.
This section is included so the checklist retains the explanatory text, tables, appendices, glossary, references, and recommendations from the source repository.

### Source file: en/0x00-Header.yaml
...
### Source file: en/0x01-Frontispiece.md
...
### Source file: en/0x02-Preface.md
...
### Source file: en/0x03-What-is-the-ASVS.md
...
### Source file: en/0x04-Assessment_and_Certification.md
...
### Source file: en/0x05-For-Users-Of-4.0.md
...
### Source file: en/0x10-V1-Encoding-and-Sanitization.md
...
### Source file: en/0x11-V2-Validation-and-Business-Logic.md
...
### Source file: en/0x12-V3-Web-Frontend-Security.md
...
### Source file: en/0x13-V4-API-and-Web-Service.md
...
### Source file: en/0x14-V5-File-Handling.md
...
### Source file: en/0x15-V6-Authentication.md
...
### Source file: en/0x16-V7-Session-Management.md
...
### Source file: en/0x17-V8-Authorization.md
...
### Source file: en/0x18-V9-Self-contained-Tokens.md
...
### Source file: en/0x19-V10-OAuth-and-OIDC.md
...
### Source file: en/0x20-V11-Cryptography.md
...
### Source file: en/0x21-V12-Secure-Communication.md
...
### Source file: en/0x22-V13-Configuration.md
...
### Source file: en/0x23-V14-Data-Protection.md
...
### Source file: en/0x24-V15-Secure-Coding-and-Architecture.md
...
### Source file: en/0x25-V16-Security-Logging-and-Error-Handling.md
...
### Source file: en/0x26-V17-WebRTC.md
...
### Source file: en/0x90-Appendix-A_Glossary.md
...
### Source file: en/0x91-Appendix-B_References.md
...
### Source file: en/0x92-Appendix-C_Cryptography.md
...
### Source file: en/0x93-Appendix-D_Recommendations.md
...
### Source file: en/0x94-Appendix-E_Contributors.md
