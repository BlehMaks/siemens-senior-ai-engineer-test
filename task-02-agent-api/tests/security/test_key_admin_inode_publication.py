...
def test_parent_rename_during_final_fsync_removes_plaintext(
    ...
    assert _cleanup_entries(displaced) == []


def test_failure_quarantines_before_unlinking_the_owned_name(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    output = tmp_path / "generated.key"
    real_fsync = os.fsync
    ...
    def record_unlink(target: str, **kwargs: Any) -> None:
        unlinked_names.append(target)
    ...
    assert output.name not in unlinked_names
    assert all(name.startswith(".api-key-cleanup-") for name in unlinked_names)
    assert not output.exists()
    assert _cleanup_entries(tmp_path) == []
